...
//    t_mu3e->SetBranchAddress("hit_timestamp", &timestamp);
    ...
    for (unsigned int i = 0; i < segs_entries; i++) {
        ...
        if (trajp / mc_p < 0.99 || trajp / mc_p > 1.01) {
            ...
            //is rec_p really the same as mc_p ??
    ...
    //TODO: Update mean functions in order to make more sense
    float p_inv_rec_error_mean = std::accumulate(p_inv_rel_errors.begin(), p_inv_rel_errors.end(),
    decltype(p_inv_rel_errors)::value_type(0)) / p_inv_rel_errors.size();

    float p_rec_error_mean = std::accumulate(p_rel_errors.begin(), p_rel_errors.end(),
            decltype(p_rel_errors)::value_type(0)) / p_rel_errors.size();

    ////PRINT THE STATS
    cout << endl << endl << "---General Stats---\n" << endl;
    cout << "p_mc and trajp fail count: " << p_fail_count << ", total: " << segs_entries << ", rate: "
