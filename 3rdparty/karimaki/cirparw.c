/* cirparw.f -- translated by f2c (version 20191129).
...
/* Subroutine */ int cirparw_(real *x, real *y, real *w, integer *n, real *tr,
	 real *cdf)
{
...
/*     shift to first point */
	...
	*sw += (doublereal) w[i__];
	*sx += (doublereal) w[i__] * ((doublereal) x[i__] - (doublereal) x[1])
		;
/* L20: */
	*sy += (doublereal) w[i__] * ((doublereal) y[i__] - (doublereal) y[1])
		;
    }
    cx = *sx / *sw;
    cy = *sy / *sw;
...
/*     ... using shifted coordinates */
	...
	r2 = xi * xi + yi * yi;
	*sxx += (doublereal) w[i__] * (xi - cx) * (xi - cx);
	*sxy += (doublereal) w[i__] * (xi - cx) * (yi - cy);
	*syy += (doublereal) w[i__] * (yi - cy) * (yi - cy);
	*sxr2 += (doublereal) w[i__] * xi * r2;
	*syr2 += (doublereal) w[i__] * yi * r2;
	*sr2 += (doublereal) w[i__] * r2;
/* L30: */
	*sr4 += (doublereal) w[i__] * r2 * r2;
    }
